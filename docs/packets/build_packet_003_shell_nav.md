...
**Adjudication:** PASS shape = two files; the `window.INBOX` object's contents byte-identical to `window.TEXTWALL`'s (diff shows only the declaration line changing); `TEXTWALL` and `tw-theme` gone; five nav buttons with the flag false; the segmented control present and inert; `body` bottom padding raised. FAIL on: any `supabase/` hunk; a sixth nav button with `SHOW_EVENTS` false; an Unsorted segment; a working Matrix view; `PUBLIC_HOST` removed in this unit (that is U003-C); any change to `counts`/`visible`/`renderGrocery` semantics; a new dependency; a `<script>` tag added; version not bumped to `003-A`.
